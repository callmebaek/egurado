-- ============================================
-- 사용자 데이터를 새 Auth ID로 마이그레이션
-- 기존 profiles ID → 새 Auth ID로 전환
-- ============================================

-- 사용자 데이터를 새 Auth ID로 마이그레이션하는 함수
...
BEGIN
    ...
    RAISE NOTICE '% stores migrated from % to %', 
    ...
    RAISE NOTICE '% metric_trackers migrated from % to %', 
...
'기존 profiles ID를 새 Auth ID로 마이그레이션. stores, metric_trackers의 user_id도 함께 업데이트.';

-- 완료 메시지
SELECT 'migrate_user_to_new_auth_id function created successfully' AS status;
