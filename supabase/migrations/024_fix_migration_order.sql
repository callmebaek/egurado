-- ============================================
-- Fix migration order: Create profiles BEFORE updating stores
...
BEGIN
    ...
    -- 1. 기존 프로필에서 credit/quota 정보 가져오기 (테이블명 명시)
    SELECT 
    ...
    RAISE NOTICE 'New profile % created', p_new_id;
    
    -- 3. stores의 user_id를 새 ID로 업데이트
    UPDATE stores
    ...
    -- 4. metric_trackers의 user_id를 새 ID로 업데이트
...
'기존 profiles ID를 새 Auth ID로 마이그레이션. Foreign Key 제약 위반 해결을 위해 profiles를 먼저 생성한 후 stores와 metric_trackers 업데이트.';

-- 완료 메시지
SELECT 'Migration function order fixed - profiles created BEFORE stores update' AS status;
